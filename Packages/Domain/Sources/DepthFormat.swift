...
public enum DepthFormat {
    ...
    /// Chart axis caption for the current depth unit, e.g. `"Depth (m)"`. The
    /// "Depth" caption is localized (against Domain's own bundle); the unit
    /// symbol interpolates in and stays a bare symbol.
    public static func axisLabel(_ units: UnitPreference = .current) -> String {
        String(localized: "Depth (\(unitLabel(units)))", bundle: .module)
    }
}
