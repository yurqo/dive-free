...
/// depth, heart-rate, and water-temperature series, for Strava's `/uploads`
/// endpoint. FIT is the only single-file format that carries **both** a
/// `total_calories` figure (which GPX can't) **and** a per-point temperature
/// stream (which TCX can't) — so the dive's calories and water-temperature
/// graph both reach Strava.
///
...
/// is set to `swimming`; the uploader also forces `Swim` on the activity after.
///
...
public enum StravaFIT {
    ...
    public static func build(_ session: DiveSession) -> Data? {
        ...
        // session (global 18): Strava reads total_calories + sport from here.
    ...
    private struct Writer {
        ...
        mutating func u16(_ v: UInt16) { bytes += StravaFIT.le16(v) }
        mutating func u32(_ v: UInt32) { bytes += StravaFIT.le32(v) }
        mutating func i32(_ v: Int32) { bytes += StravaFIT.le32(UInt32(bitPattern: v)) }
    }
}
